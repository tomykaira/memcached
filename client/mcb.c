 ...
 *
 * Copyright (C) 2008-2011  suzuki hironobu
 *
...
static void master_thread(void)
{
    ...
    fprintf(stdout, "\tsend %d command a thread, total %d command\n",
	    sysval.command_num, sysval.command_num * sysval.thread_num);
...
static int connect_mc()
{
	...
	if (sysval.type == UNIX_SOCKET)
	    fprintf(stderr, "\tHint => Check permition of unix socket parh(%s).\n", sysval.addr);
	exit(-1);
    }
    assert(fd != -1);
    return fd;
...
static void usage(void)
{
    fprintf(stderr, "mcb -- simple memcached benck mark\n");
    ...
    fprintf(stderr, "\t\t-p server_port[%d]\n", DEFAULT_PORT);
    fprintf(stderr, "\t\t-T connection Type {TCP|UDP|UNIX_SOCKET}[TCP]\n");
    fprintf(stderr, "\t\t-t number_of_thread[%d] <1-%d>\n", DEFAULT_THREADS, MAX_THREADS);
    fprintf(stderr, "\t\t-n number_of_command[%d] <1-%d>\n", DEFAULT_COMMANDS, MAX_COMMANDS);
    fprintf(stderr, "\t\t-m max_key[%d] <1-%d>\n", DEFAULT_MAX_KEY, MAX_KEY);
    fprintf(stderr, "\t\t-l average data_length[%d] <1-%d>\n", DEFAULT_DATA_LEN, MAX_DATA_LEN);
    fprintf(stderr, "\t\t-v               :verbose\n");
#ifdef _DEBUG_
    fprintf(stderr, "\t\t-d debug_level   :debug[0-2]\n");
#endif
    fprintf(stderr, "\t\t-s               :send and disconnect each command\n");
...
int main(int argc, char **argv)
{
    ...
    for (i = 0; i < sysval.thread_num; i++)
	if (pthread_create(&connector_tptr[i], NULL, (void *) connector_thread, (void *) i) != 0) {
	    elog("pthread_create() error");
	    exit(-1);
	}
